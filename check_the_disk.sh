#!/usr/bin/env bash
# vars #
disk="/tmp" # disk to monitor
current_usage=$(df -h | grep ${disk} | awk {'print $5'}) # get disk usage from monitored disk
max_usage="85%" # max 90% disk usage
mail="o.mustafa@ligadata" # mail to sent alert to

# functions #
function max_exceeded() {

    # tell that mail is being sent
    echo "Max usage (${max_usage}) exceeded. Your disk usage is it at ${current_usage}., the script will delete the files automatically, Trying to send mail-alert..."
